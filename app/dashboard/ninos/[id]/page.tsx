...
// 1. IMPORTAMOS EL MOTOR OFFLINE
import { guardarEnCola } from '@/lib/offline-sync';
import { ArrowLeft, Activity, Save, X, AlertTriangle, Pill, Clock, UserPlus, Trash2, User } from 'lucide-react';
...
// Nuevo tipo para el cuidador en la lista
interface Cuidador { 
  id: string; 
...
export default function FichaNinoPage() {
  ...
  // Estado para la lista de cuidadores (aunque solo permitiremos 1)
  const [cuidadores, setCuidadores] = useState<Cuidador[]>([]);
  
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [showModalCuidador, setShowModalCuidador] = useState(false); // Modal Vinculación
  ...
  const fetchData = async () => {
    ...
    // 4. Cargar Cuidadores (Join complejo para sacar nombres)
  ...
  // --- FUNCION NUEVA: AGREGAR CUIDADOR (CON REGLA DE 1 SOLO) ---
  const handleAddCuidador = async (e: React.FormEvent) => {
    e.preventDefault();

    // REGLA DE NEGOCIO: UN NIÑO SOLO PUEDE TENER UN CUIDADOR
    ...
    // 2. Verificar si ya existe como "Cuidador" en la tabla tbl_cuidadores
    let cuidadorId = null;
    ...
    if (existeCuidador) {
      ...
      // Si no es cuidador aun (era solo usuario), creamos su perfil de cuidador
      ...
      if (error) { alert("Error al crear perfil cuidador"); return; }
      cuidadorId = nuevo.id;
    }

    // 3. Crear el vínculo en la tabla intermedia
    ...
    if (errorVinc) {
      ...
      setSearchEmail('');
      fetchData(); // Recargar para ver al nuevo papá
    }
  };

  // --- FUNCION NUEVA: DESVINCULAR CUIDADOR ---
  const desvincular = async (idRelacion: number) => {
    if(!confirm("¿Seguro que deseas quitar el acceso a este cuidador?")) return;
    
    await supabase.from('tbl_ninos_cuidadores').delete().eq('id', idRelacion);
    fetchData(); // Recargar la lista (quedará vacía para permitir agregar otro)
  };
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
            ...
            {/* Solo mostramos el botón si NO hay cuidadores (Regla 1 a 1) */}
            {cuidadores.length === 0 && (
              <button 
                ...
                {cuidadores.map(c => (
                    <div key={c.relacion_id} className="flex justify-between items-center p-3 bg-blue-50/50 border border-blue-100 rounded-lg">
                        ...
                        <button 
                          ...
                          title="Desvincular (Permitirá agregar otro)"
                        >
                            <Trash2 size={18} />
                        </button>
                    </div>
